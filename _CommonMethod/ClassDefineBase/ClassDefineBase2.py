...
class Person:
    ...
    def __str__(self):
        '''print(instance)时 实例的显示
            这里存在硬编码问题：如 类名 的显示，无法显示子类的类名，以及属性写死的问题
        内省工具：
            类名：instance.__class__.__name__
            属性字典：instance.__dict__  (instance.__dict__.keys())
        通过内省工具将 实例显示 封装成一个通用工具 classtools.py中的AttrDisplay类 --> 3_ClassDefineBase3.py
        '''
