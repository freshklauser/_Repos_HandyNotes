...
class Person(ClassTreeAttributesDisplay):
    ...
    def __getattr__(self, attrname):
        ...
        if attrname in self.__dict__.keys():
            ...
            # 新增性并返回, 用self.__dict__[attrname]而不是self.attrname
            self.__dict__[attrname] = 'sales department'
            return self.__dict__[attrname]
            # self.attrname = 'sales'                    # 'attrname': 'sales', 属性名编程了 attrname 而不是预期的 department
            # return self.attrname
        else:
            raise AttributeError(
                "Attribute '{}' is not existed".format(attrname))
...
class Manager(Person):
    def __init__(self, name, pay):                      # name, pay 本地变量
        ...
        # 调用Person的__init__方法初始化Manager, 继承Person的属性
        # 调用父类super class 的 __init__ 方法来继承 父类 的属性
        super().__init__(name, 'Manager', pay)

    @decorator_timmer
    def giveRaise(self, percent, bonus=0.1):
        # Person.giveRaise(self, percent + bonus)       # 需要 传入 self,
        # self代表的是Person类的实例
        # 不需要传入self, super()相当于实例化了一个 super 类
...
if __name__ == '__main__':
    ...
    # print(x.__mro__)
    # 元组：(<class '__main__.Manager'>, <class '__main__.Person'>, <class
    # 'classtools.AttrDisplay'>, <class 'object'>)
    ...
    # 不加 @property: lastName方法需要按照方法的规则调用， Person().lastName()   ---
    # lastName() 后的()不能少
    print("tom's lastName: ", tom.lastName())
    # 加 @property: getJob方法利用属性进行了装饰, 需要按照属性的规则调用， Person().getJob   ---
    # getJob后不能加()
