...
class decor_tracer:
    ...
    def __call__(self, func):
        ...
        def inner(*args, **kwargs):
            self.call += 1
            print(
                "Function '{}' is running within decorator '{}'.".format(
                    func.__name__,
                    self.__class__.__name__))
            print(
                "Function '{}' was called {} times".format(
                    func.__name__, self.call))
    ...
    def __call__(self, func):
        ...
        def inner(*args, **kwargs):
            ...
            msg = "< Timestamp:{:.0f}(ms),\ttime-consumed: {:.4f}(s) for function '{}' >\n ".format(
                time.time() * 1000, time_consumed, func.__name__)
...
if __name__ == '__main__':
    ...
    # 类装饰器需要()  --> sum_ab = logitimmer().__call__(sum_ab)
    @decor_logitimmer(log_suffix='.log')
    def sum_ab(a, b):
        time.sleep(0.2210)
        return a ** 2 + a * b + b ** 2
    ...
    print(res, '-----+++++++++++++++++')
    print(type(res))
    # print(sum_ab.__name__)      # def inner上没有@wraps(func)时，输出inner,
    # 有@wraps时，输出sum_ab
    import inspect
    print()
    # 获得函数源码
